...
export default function Colares() {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        const response = await fetch('https://lime-dragonfly-325155.hostingersite.com/products.php?categoria=colares');
        if (!response.ok) {
  ...
  return (
    <div className="mb-10">
      <Head>
        <title>Colares - Beauty Style</title>
          ...
          filteredProducts.map((product) => {
            ...
            return (
              <ItemProduct
                ...
                page="colares"
                title={product.title}
                imagem={product.imageUrl || `/products/colares/${product.id}.webp`} // Usando imagem opcional
